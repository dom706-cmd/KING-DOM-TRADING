...
OUT=/tmp/orb_v6_validation
LOG=/tmp/orb_v6_validation.log
rm -rf "$OUT"
mkdir -p "$OUT"
...
python - <<'PY'
import json
d=json.load(open('/tmp/orb_v6_validation/live_quotes.json'))
...
start=json.load(open('/tmp/orb_v6_validation/orb_start.json'))
job_id=start.get("job_id") or start.get("id")
assert job_id, "ORB start missing job_id"
open('/tmp/orb_v6_validation/orb_job_id.txt','w').write(job_id)
...
for i in range(120):
    ...
    with open('/tmp/orb_v6_validation/debug_orb_final.json','w') as f:
...
python - <<'PY'
import json
d=json.load(open('/tmp/orb_v6_validation/debug_orb_final.json'))
job_id=open('/tmp/orb_v6_validation/orb_job_id.txt').read().strip()
...
assert (rc.get("intraday_or_window_missing_open") or 0) == 0, "ORB missing_open not zero"
...
python - <<'PY'
import json, time, urllib.request, urllib.parse
start=json.load(open('/tmp/orb_v6_validation/rr_start.json'))
job_id=start.get("job_id") or start.get("id")
assert job_id, "RR start missing job_id"
open('/tmp/orb_v6_validation/rr_job_id.txt','w').write(job_id)
url="http://127.0.0.1:8050/api/debug_last_scan?job_id=" + urllib.parse.quote(job_id)
for i in range(120):
    with urllib.request.urlopen(url) as r:
        d=json.load(r)
    print({"i": i, "job_id": d.get("job_id"), "strategy": d.get("strategy"), "status": d.get("status"), "running": d.get("running")})
    with open('/tmp/orb_v6_validation/debug_rr_final.json','w') as f:
        json.dump(d,f,indent=2)
    if d.get("job_id") == job_id and not d.get("running") and d.get("status") in ("completed","failed","done"):
        break
    time.sleep(2)
...
python - <<'PY'
import json
d=json.load(open('/tmp/orb_v6_validation/debug_rr_final.json'))
job_id=open('/tmp/orb_v6_validation/rr_job_id.txt').read().strip()
...
tar -czf ~/Desktop/orb_v6_validation_bundle.tgz "$OUT" "$LOG"
echo "Wrote ~/Desktop/orb_v6_validation_bundle.tgz"
